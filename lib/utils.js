...
globalThis.DataUtil = {
	...
	_merging: {},
	_merged: {},
	jsonCacheName: '5etoolsJSON-cache',
	...
	/**
	 * Function to fetch JSON content in a new tab and return it to the source tab
	 * @param {string} jsonUrl
	 * @returns jsonData
	 * @description This function opens a new tab to fetch JSON content, avoiding CORS issues.
	 * It waits for the new tab to load, fetches the JSON content, and then closes the new tab.
	 * The fetched JSON data is returned to the source tab.
	 */
	async loadJSONInNewTab (jsonUrl) {
		// 1st try and get from cache
		const cache = await caches.open(this.jsonCacheName);

		// Encode the URL to use as the cache key
		const cacheKey = encodeURIComponent(jsonUrl);

		// Check if the JSON content is already in the cache
		...
		await this.checkTabLoaded(newTab);

		try {
			// Fetch JSON content in the new tab to avoid CORS issues
			const response = await newTab.fetch(jsonUrl);
			const jsonData = await response.json();

			// Close the new tab
			newTab.close();
			// Store the JSON content in the cache
			await cache.put(cacheKey, new Response(JSON.stringify(jsonData)));

			console.log('JSON data copied to source tab:', window.jsonDataFromNewTab);
			// Copy the JSON object to a variable in the source tab
			return jsonData;
		} catch (error) {
			console.error('Error fetching JSON in new tab:', error);
		}
		return null;
	},
	 ...
	 * @param {object} newTab the response from window.open
	 * @returns {Promise} A promise that resolves when the new tab is loaded
	 * @description This function checks if the new tab has loaded by checking the document's readyState.
	 * If the new tab is closed or the document is not loaded within 100 retries, it rejects the promise.
	 * It uses a recursive function to check the loading state of the new tab.
	 * @private
	 */
	async checkTabLoaded (newTab) {
		await new Promise((resolve, reject) => {
			this.checkTabLoadedResolver(newTab, resolve, reject);
		});
	},

	/**
	 * Recursive check for if the new tab has loaded
	 * @param {object} newTab the response from window.open
	 ...
	 * @description This function checks if the new tab has loaded by checking the document's readyState.
	 * If the new tab is closed or the document is not loaded within 100 retries, it rejects the promise.
	 * It uses a setTimeout to check the loading state every 100 milliseconds.
	 * @private
	 */
	checkTabLoadedResolver (newTab, resolve, reject, retries = 0) {
		if (newTab.closed || retries > 100) {
			reject(new Error('New tab was closed before loading.'));
		} else if (newTab.document.readyState === 'complete') {
			resolve();
		} else {
			setTimeout(this.checkTabLoadedResolver, 100, newTab, resolve, reject, retries + 1);
		}
	},
